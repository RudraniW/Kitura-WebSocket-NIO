...
extension KituraTest {
    ...
    func sendFrame(final: Bool, withOpcode: Int, withMasking: Bool=true, withPayload: NSData, on channel: Channel, lastFrame: Bool = false, compressed: Bool = false, contextTakeover: ContextTakeover? = nil) {
        var clientTakeover = true, serverTakeover = true
        if let contextTakeover = contextTakeover {
            clientTakeover = (contextTakeover == .client || contextTakeover == .both)
            serverTakeover = (contextTakeover == .server || contextTakeover == .both)
        }
